...
<br>
Нека погледнем класа Exception от близо. `__init__` методът на `BaseException` (който Exception директно наследява) всъщност има следната сигнатура - `__init__(*args)`. Какво значи тази звезда преди `args`? Значи, че на метода можем да подадем 0 или n на брой аргумента. Бърз пример:
